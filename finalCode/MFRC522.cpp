...
void MFRC522::PCD_Init() {
	...
	if (_resetPowerDownPin != UNUSED_PIN) {
		...
		if (PINB & (1 << PINB4)) {
			...
			DDRB |= (1 << DDB4);		    // Now set the resetPowerDownPin as digital output.
			PORTB &= ~(1 << PORTB4);	// Make sure we have a clean LOW state.
			delayUs(2);	        // 8.8.1 Reset timing requirements says about 100ns. Let us be generous: 2μsl
			PORTB |= (1 << PORTB4);		    // Exit power down mode. This triggers a hard reset.
